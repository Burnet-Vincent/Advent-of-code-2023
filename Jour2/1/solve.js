const unsolvedcode = `Game 1: 9 red, 2 green, 13 blue; 10 blue, 2 green, 13 red; 8 blue, 3 red, 6 green; 5 green, 2 red, 1 blue
...
function getValues(row){
    ...
    values.forEach(value => {
        let val = Number(value.match(/(\d+)/)[0]);
...
listlines.forEach(line => {
    let ind = line.indexOf(":") + 1;
    let tmpstring = line.substring(0, ind);
    let level = Number(tmpstring.match(/(\d+)/)[0]);
    red = 0, green = 0, blue = 0;

    line = line.substring(ind);
    while((ind = line.indexOf(";") + 1) != 0){
        tmpstring = line.substring(0, ind);
        getValues(tmpstring);
        line = line.substring(ind);
    }

    getValues(line);
    if(red <= 12 && green <= 13 && blue <= 14) total += level;
});
