const unsolvedcode = `........................617.........123...........341.........................293..................38..19.753..................533..........
...
listlines.forEach((line, index) => {
    let values = [...line.matchAll(/(\d+)/g)];
    values.forEach(value => {
        let val = value[0];
        let ind = value["index"];

        if(ind != 0 && line.charAt(ind-1) == '*'){
            stars.push([[index, ind-1], Number(val)]);
        }

        if(ind + val.length != line.length && line.charAt(ind + val.length) == '*'){
            stars.push([[index, ind + val.length], Number(val)]);
        }
        ...
        if(index != 0){
            ...
            let sub = listlines[index-1].substring(tmpind, ind + val.length + 1);
            if((tmp = sub.indexOf("*")) != -1){
                stars.push([[index-1, tmpind + tmp], Number(val)]);
            }
        }
...
while(stars.length != 0){
    ...
    let second;
    let check = 0;
    while(stars[0] && equalsCheck(first, stars[0])){
        second = stars.shift();
        check++;
    }
    if(check == 1){
