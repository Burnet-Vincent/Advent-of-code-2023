const unsolvedcode = `-7-|-J-F7F--7.|77.77FFJ7F77..7FJ-77-FF7FF-777F-77--L7FFJJF77FF|7F7F--FJ7-7-77-7-7.F-7--F7.F7F---7F7FF7FL|7-7-F--F7-FF7-|.|77LL-FFL7J7F-F-F7.
...
Array.prototype.myIndexOf = function(arr){
    ...
    for(i = 0; i < this.length; i++){
        if(JSON.stringify(arr) == JSON.stringify(this[i])) return i;
    }
    return -1;
}

Array.prototype.letterAt = function (x, y){
    if(x < 0 || x >= listlines[0].length || y < 0 || y >= listlines.length) return -1;
    return this[y].charAt(x);
}

Array.prototype.getPrecY = function (ind){
    ind--;
    if(ind < 0) ind = this.length - 1;
    return this[ind][1];
}

Array.prototype.getNextY = function (ind){
    ind++;
    if(ind >= this.length) ind = 0;
    return this[ind][1];
}

String.prototype.nbZero = function (indexline){
    let val = [...this.matchAll(/(0+)/g)];
    let res = 0;
    val.forEach(element => {
        if(element[0].length > 1){
            for(let i = 0; i < element[0].length; i++){
                let j = 0;
                let index = boucle.myIndexOf([element.index+i, indexline]);
                let deb = boucle.getPrecY(index);
            
                if(deb == indexline){
                    deb = boucle.getNextY(index);
                    while(boucle.getPrecY(index-j) == indexline){i++;j++;}
    
                    if(deb != boucle.getPrecY(index-j)) res++;
                }else{
                    while(boucle.getNextY(index+j) == indexline){i++;j++;}
    
                    if(deb != boucle.getNextY(index+j)) res++;
                }
            }
...
function loop(x, y, from){
    let ret = Array();
    while(1){
        let char;
        if((char = listlines.letterAt(x,y)) == -1) return;
        ret.push([x,y]);
        switch(char){
            case "|":
                if(from == "h") {y++;from="h";}
                else if(from == "b") {y--;from="b";}
                else return;
                break;
            case "-":
                if(from == "d") {x--;from="d";}
                else if(from == "g") {x++;from="g";}
                else return;
                break;
            case "L":
                if(from == "h") {x++;from="g";}
                else if(from == "d") {y--;from="b";}
                else return;
                break;
            case "J":
                if(from == "g") {y--;from="b";}
                else if(from == "h") {x--;from="d";}
                else return;
                break;
            case "7":
                if(from == "g") {y++;from="h";}
                else if(from == "b") {x--;from="d";}
                else return;
                break;
            case "F":
                if(from == "d") {y++;from="h";}
                else if(from == "b") {x++;from="g";}
                else return;
                break;
            case ".":
                return;
...

{
    ...
    while(S == undefined){
        let x;
        if((x = listlines[y].indexOf("S")) != -1) S = [x, y];
        y++;
    }
}
...
boucle.forEach((coo, index) => {
    let i = coo[1];
    listlines[i] = listlines[i].substring(0, coo[0]) + "0" + listlines[i].substring(coo[0]+1);
});

listlines.forEach((line, index) => {
    if(index != 0 && index != listlines.length - 1){
        for(let x = 1; x < line.length-1; x++){
            if(line.charAt(x) != "0" && listlines[index].substring(0, x).nbZero(index) % 2 == 1) {
                total++;
            }
        }
    }
